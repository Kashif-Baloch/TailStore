...
export const getItems = async (l, p) => {
  const data = await fetch(
    `http://localhost:3000/apis/products?category=tshirt&mode=cwise&limit=${l}&page=${p}`,
    {
      method: "GET",
      headers: {
      ...
      <section className="text-gray-600 body-font mt-20">
        <div className="container px-5 py-24 mx-auto">
              ...
              myProducts.Data.map((elem, indexm) => {
                return (
                  <div key={indexm} className="w-[85%] md:w-[16rem]">
                    <div className="md:h-[32rem] relative rounded-lg cardc shadow-lg hover:shadow-xl cursor-pointer overflow-hidden m-2 dark:bg-gray-700">
          ...
          <div className="justify-center gap-12 mt-20 items-center flex flex-wrap">
            <Link
              href={`/tshirts?page=${page <= 1 ? 1 : page - 1}`}
              type="button"
              disabled={page <= 1}
            ...
            <Link
              href={`/tshirts?page=${page == pages ? page : page + 1}`}
              type="button"
